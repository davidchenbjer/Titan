...
public class TicketMasterAPI implements ExternalAPI {
	...
	private static final String API_KEY = "PoBhNxi9rR0N7fyfBHCCXVLNH0EfJo89";
	
	@Override
	// rewrite the search function in ExternalAPI
	public List<Item> search(double lat, double lon, String term) {
		...
		if (term == null) {
			term = DEFAULT_TERM; // term is the keyword
		}
		...
		try {
			...
			String inputLine;
			StringBuilder response = new StringBuilder(); // string is easy to use when parsing and cleaning
			while ((inputLine = in.readLine()) != null) { // line by line
				response.append(inputLine);
			}
			in.close();
	
			JSONObject responseJson = new JSONObject(response.toString()); //JSON could parse a string to JSONobject
	...
	public static void main(String[] args) {
		...
		// Toronto, CA
		tmApi.queryAPI(43.6532, -79.3832);
